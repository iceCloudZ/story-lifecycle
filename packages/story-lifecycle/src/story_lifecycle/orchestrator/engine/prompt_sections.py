...
# ---------------------------------------------------------------------------
# 公司 SOP 文档模板段(印闪需求处理流程 SOP V1.7,2026-08-28 对齐)
# 静态内容、无 DB 依赖 —— 纯函数,verify/design 阶段注入。
# ---------------------------------------------------------------------------

_TEST_REPORT_TEMPLATE = """## 测试报告模板(公司 SOP《系统测试报告》— 必须遵守)

`story/test-report.md` 按公司模板结构产出(SOP 冒烟门:开发做 AI 系统测试,**整体
通过率 ≥80%** 才流转测试,报告附 TAPD)。五个部分一个不能少:

```
# 系统测试报告
报告版本:V1.0
测试对象:<系统/服务名 + 版本号>
测试周期:YYYY年MM月DD日 - YYYY年MM月DD日
测试负责人:<负责人>

一、测试概述
<本次测试针对的新增功能与核心流程,覆盖哪些主业务链路/配套功能/基础异常场景,一两段>

二、用例执行概览
| 用例类型 | 总用例数 | 通过数 | 失败数 | 通过率 |
|---|---|---|---|---|
| 核心主流程 | | | | |
| 重要功能点 | | | | |
| 基础异常场景 | | | | |
...
三、缺陷汇总说明
| 缺陷ID | 缺陷描述 | 所属模块 | 等级 | 处理状态 |
(无缺陷则写明"本次测试未发现缺陷")

四、核心指标验证
<逐条列实测指标:接口响应耗时/数据准确性/流程成功率等,按本 story 实际验证项>

五、最终测试结论
<目标是否达成/是否同意按计划上线/遗留问题与闭环计划>
```

**硬性要求**:
- 用例数字必须来自**真实执行的测试**(hc-pytest journeys 结果、接口实测、APP 实机
  走查),不允许编造;每行通过率 = 通过数/总数,汇总行 = 各行加和。
- 纯后端/无界面的 story,「APP UI 冒烟」行写「不适用」并一句话说明;涉及 C 端 APP
  的 story,优先在安卓模拟器(adb 已接)上对改动页面做真机冒烟后填数。
- 结论必须与合计通过率一致:≥80% 才能写"同意按计划上线",<80% 如实写未达冒烟门。
"""


_SPEC_SKELETON_TEMPLATE = """## 技术设计文档骨架(公司 SOP 研发设计评审 — 必须遵守)

`story/spec.md` 按公司《技术设计文档》要求组织(SOP:复杂/高风险需求须研发设计评审,
评审通过的文档附 TAPD)。spec.md 至少覆盖四节,顺序如下:

1. **整体架构**:本改动涉及的服务/模块/数据流(mermaid 图 + 一段文字)
2. **核心改动点**:逐条列「改哪个服务/文件/接口 → 改什么 → 为什么这么改」
3. **影响范围**:受影响的接口/表/MQ/前端/下游服务;并明确「不影响什么」
4. **风险与应对**:风险点 + 对策(灰度/回滚/兼容性/数据迁移)

调研结论(现状代码怎么走的)作为第 1/2 节的支撑材料并入,不单独立节。
"""


def build_test_report_template_section(stage: str) -> str:
    """verify 阶段注入公司系统测试报告模板。stage 门控留给调用方。"""
    if stage != "verify":
        return ""
    return _TEST_REPORT_TEMPLATE


def build_spec_skeleton_section(stage: str) -> str:
    """design 阶段注入公司技术设计文档骨架。stage 门控留给调用方。"""
    if stage != "design":
        return ""
    return _SPEC_SKELETON_TEMPLATE
