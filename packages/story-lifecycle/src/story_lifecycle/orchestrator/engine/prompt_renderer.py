...
def _build_stage_contract(stage: str, state: StoryState) -> str:
    ...
    return (
        ...
        f"- Story Key: {key}\n"
        f"- Stage: {stage}\n"
        "- 完成判定:**必须调 `story tool declare`**(下方 Required artifacts),编排器据此推进。\n"
        "  编排器只认 declare 信号,不看文件存在 —— 只写文件不 declare,stage 永远卡住。\n"
        "- 落地方式:用 `story tool declare <doc_type> <path>`(原子写+版本化+触发感知)。\n"
        "  可以先 Write 写草稿,确认内容完整后再 declare --content 落地。\n"
        "- **不要**自己写 done.json(旧协议,已废)。\n"
        "- 不要在 declare 成果物后继续往后跑下一 stage(编排器负责推进)。\n\n"
        "### Required artifacts(本 stage 完成信号 — 必须逐个 declare)\n\n"
        f"{artifact_lines}\n\n"
