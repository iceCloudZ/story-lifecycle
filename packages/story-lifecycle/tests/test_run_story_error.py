...
设计13:run_story 内部调 ``scheduler.drive_story_sync``(同步驱动编排引擎),
测试 monkeypatch 的目标从 planner.continue_orchestrator_agent 换成 drive_story_sync。
"""
...
def test_run_story_no_error_does_not_mark_failed(monkeypatch):
    ...
    monkeypatch.setattr(
        "story_lifecycle.orchestrator.scheduler.drive_story_sync", ok
    )
...
def test_run_story_logs_recovery_action_on_raise(monkeypatch):
    """drive 抛错 → run_story 调 decide_recovery 落 recovery_action 事件(层3)。"""
    ...
    monkeypatch.setattr(
        "story_lifecycle.orchestrator.scheduler.drive_story_sync", boom
    )
...
def test_run_story_rescues_on_retryable_failure(monkeypatch):
    """层3 rescue Handler:有 _agent_actions + 可恢复失败 → 换 adapter 重跑,drive 被调 2 次。"""
    ...
    monkeypatch.setattr(
        "story_lifecycle.orchestrator.scheduler.drive_story_sync", flaky
    )
