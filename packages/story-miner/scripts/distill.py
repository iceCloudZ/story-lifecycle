...
# pick 3 successful claude sessions: turns 5-25, has tool+result+code, errs low, ws=hc-all
cand=[]
for sid,evs in by.items():
    m=meta.get(sid,{})
    if not sid.startswith('claude'): continue
    if m.get('ws')!='hc-all': continue
...
out.append(f"> 候选「成功轨迹」(claude/hc-all, 5-25轮, ≥10工具, 有代码, 低错误): **{len(cand)}** 个\n")
out.append("## 候选会话\n")
...
open(r'D:/hc-all/.claude/tmp/cache/d2_distill.md','w',encoding='utf-8').write('\n'.join(out))
