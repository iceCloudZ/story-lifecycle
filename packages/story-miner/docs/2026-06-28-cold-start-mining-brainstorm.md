...
> 状态：**三大设计已确认 + 结果轴一期已实施验收**（A/B/C/D/E 全 done，bug 磁铁代码 11/11）；剩余 = 点3 注入 / 点6 门禁落地接通 + merge 策略
...
## 结果轴一期最终设计：bug 关联图谱（已收敛 2026-06-28）

> 状态：**已确认设计**。结果轴（outcome 层）一期 = 把 TAPD bug ↔ story ↔ git commit 关联成图，按 `task_type` 聚合成飞轮知识。补"过程轴（点 3）"之外的结果半边。

### 一期管线总览（A/B/C/D/E）

```
数据源                关联/挖掘 (脚本)                产物 (scripts/out/)
─────────────       ─────────────────────           ─────────────────────
TAPD                A bug_story_graph       ✅ ──► bug_story_graph.json
 stories 150         反向 get_related_bugs          (precise 234 link
 bugs 142            [hard·正向 story_id 全空]       +severity/status/time)
 iterations 200     C classify_task_type    ✅ ──► story_task_types.json
                     LLM 12 类打标                  (150 story → task_type)
                    D bug_iteration_links   ✅ ──► bug_iteration_links.json
                     同 sprint 撮合 [weak]          (覆盖 20%→53%·sprint 级
                                                   ·精度≈0·只粗聚合)

hc-all git 17 子仓  B story_commits         ✅ ──► story_commits.json
 story.db branch 36  branch→子仓 git log           (19 story → commits)
                    [hard]
                    E infer_magnet_commits  ✅ ──► known_magnet_commits.json
                     时间窗+语义 [weak]             (8 无 branch 磁铁 → commit)

                           ╔════════════════════════════╗
                           ║ 汇总验收  (join by story_key) ║
                           ║ 按 task_type 聚合            ║ ──► outcome_knowledge.md
                           ║ 精确集为主 · weak 只 sprint   ║     bug-rate/类 · 磁铁
                           ╚══════════════════╤═══════════╝
                                              ▼
            飞轮：knowledge_context（点3 注入）+ severity→HIGH-block（点6 门禁）
```

### 探索发现（grounded）

- **hc-all 是 multi-repo**，17 个子仓全在、可 git log（hc-order 1672、hc-user 2057、hc-risk-management 2113 commits…）。Internal GitLab：`internal-gitlab.yinshantech.cn`。
- **commit message 引 TAPD：零散，救不回来。** grep `114438` 全仓只有 hc-risk-management（101，带完整 TAPD URL）、hc-third-party（9）、hc-limit（1）；大仓 hc-order/hc-user/hc-audit/hc-message **全 0**，commit 多是 conventional 风格。
- **branch 名 `feature/<author>/<topic>_<date>`**，偶尔嵌短 id（1065487、1062174），多数 topic slug——正是 story-lifecycle `branch_rule`（没带 story_key）的产物。
- **TAPD bug 详情无"关联提交/MR"字段**（已确认）。
- **bug↔story 机制（实测修正 2026-06-28）**：TAPD bug 正向 `story_id` **全空（实测 0/142）**，`resolve_bug_parent` 正向链无效；真实机制是**反向 `get_related_bugs(story_id)`**（`tapd_api.py:99`）→ bug_id，覆盖 ~20% story（挂了的都是真 bug）。脚本 `scripts/bug_story_graph.py` 已按反向实现并跑通。

### 三条边 + 可行性（不对称）

| 边 | 档 | 机制 | 一期 |
|---|---|---|---|
| **bug ↔ story**（金矿） | hard | **反向** `get_related_bugs(story_id)`（正向 story_id 实测全空 0/142） | A ✅ 已跑通 |
| **story → commit**（feature 代码） | hard | `story_project.branch` → 子仓 `git log` 该分支（**不需 commit-msg 约定**） | A ✅ |
| **bug → 修复 commit**（修复模式） | weak | 语义 + 时间推断（无约定、无 TAPD 字段） | B 🟡 |

```
   ┌─[hard: 反向 get_related_bugs]─► story ─[hard: branch]─► feature commits
bug│                          │
   └─[weak: semantic+time]───► fix commit（修复模式）
              │
        task_type 切分 → outcome 知识
```

**间接解**：`bug →[hard] story →[hard] branch → commit`——每个 bug 对应的 **feature 代码自动可得**。唯一拿不到的是"bug 自己的修复 commit"（上线后那个后续修），那才走 weak 语义。

### 一期 A（金矿 + feature 代码，零外部依赖）

跑 `resolve_bug_parent` 全量 → 持久化 bug→story 边 → `story_project.branch` → 子仓 `git log` → 按 `task_type` 聚合：
- **bug-rate / 类型**、**bug 磁铁 feature**（纯度最高的质量信号——发布后真实缺陷，远胜 transcript 构建时报错）。
- 每个 feature 的实际代码（喂 failure-mode / 修复模式挖掘）。

### 一期 B（bug 修复 commit，weak 语义推断）

一期 A 是 B 的**缩圈器**：bug → story → 子仓 + task_type + 时间窗，候选只圈到「那个仓 + 那段」，不暴力 bug×全仓。

```
bug (title/desc/created/resolved + 经 story → 子仓 + task_type)
   │
   ▼ 缩圈：子仓 + 时间窗 [created, resolved]
   ▼ 廉价过滤（pre-LLM）：
      ├ 关键词重叠（bug 标题 ↔ commit msg）
      ├ 文件/类重叠（bug 描述 ↔ 改动文件，强信号）
      └ 时间衰减（越近 created 越可能）
   ▼ shortlist top-K
   ▼ 语义打分（embedding 批量 + 生成式 LLM 只判 close call）
   ▼
高置信 → bug→fix-commit (weak:semantic-time)    低置信 → 不挂 / memory:human
```

- **cheap-first**：关键词/文件/时间先砍大头，LLM 只判残余（呼应门禁那条"别让 LLM 干廉价活"）。
- **weak 档纪律**：只做聚合/粗粒度，不进细粒度交集，不和 hard 混。

### 历史 vs 新 bug（双轨）

- **历史 bug**：语义推断一次性回填（一期 B）。
- **新 bug**：走 hard-linkage 设计（commit-msg hook 带 bug_id / branch 带 bug_id）→ 直接 hard 边，不用猜。语义回填 + 新硬关联，两条腿。

### 产出（outcome 知识）

按 `task_type` 的：bug-rate、bug 磁铁 feature、（weak）修复模式。补上"过程↔结果"相关 → 治发散点 2 的种子偏见（哪类半自动做法 → bug 多，飞轮就能识别、不继承）。

### 待确认（建之前核）

- **TAPD bug 元数据**能否经 `tapd_source` 拉到（API key 配好没）。
- **`story_project.branch` 对历史 68 的覆盖率**（决定 story→commit 能挂多少）。
