...
- **后端服务** — 扫描各业务域的 Controller、Service、FeignClient、MQ、配置等，识别域边界和接口依赖
...
向用户展示并行扫描计划：
- "我将同时启动 5 个扫描任务：后端服务、数据库、前端、测试、CI/CD。完成后逐个向你汇报确认。这样可以吗？"

如果项目较小或用户偏好串行，则按顺序执行。

### 第 3 步：后端服务架构

扫描后端服务代码，识别业务域边界和核心逻辑，向用户确认：

1. **服务列表** — 扫描 Spring Boot 主类、application.yml，列出发现的微服务/模块
2. **业务域划分** — 根据包结构、Controller 分组识别业务域（如 order、payment、user）
3. **接口清单** — 每个 Controller 的 REST API 列表、FeignClient 远程调用关系
4. **消息队列** — MQ Producer/Consumer、Topic 列表、消费关系
5. **核心依赖** — 服务间调用链路、共享组件、公共库
6. **配置管理** — Nacos/Apollo 配置、环境变量、profile 切换

向用户提问：
- "检测到 N 个微服务/M 个业务域，划分是否合理？"
- "这些服务之间的调用关系是否符合预期？"
- "是否有服务跨了多个业务域？需要拆分还是合并？"
