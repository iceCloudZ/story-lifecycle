...
- `story tool` CLI 子命令家族(全部经 env `STORY_KEY/STORY_STAGE/STORY_WORKSPACE` 自定位 —— planner spawn 时 `build_story_spawn_env` 注入,4 条 spawn 路径全覆盖;不带参数即可工作):`workspace`(打印上下文)/ `declare <doc_type> <path>`(落成果物)/ `todo`(本 stage 缺哪些 artifacts)/ **`context`**(任务简报:PRD 摘要 `planner._read_prd_snippet` + 接手说明 `seed_context` + 本阶段 `task_actions`,纯读聚合无副作用)。`context` 的主用途是 **resume** —— resume seed 三处(`executors.py` / `spawn_recipe.py` / `sessions.py`)统一是「先跑 `story tool context` 回顾简报 → `story tool declare`」,断点续跑的 agent 靠它重新拉全貌,不靠人重黏 prompt(PTY 新会话 seed 早是一行指针「请读取 `prompt_<stage>.md`」,PRD 在执行 prompt 里也已是路径指针)。
...
1. **调度点① Stage 完成裁判 (`stage_completion.judge_stage_completion`)** — artifacts landed → **一次 LLM 三个决定** (设计12): `quality` (approve/reject/escalate) + `lifecycle_target` (「最远能到哪」,看所有累积产出,替换 `_stages_done`+`gate_satisfied` 硬编码) + `summary` (UI 展示)。**non-agentic** (no read_file/query_db tools; context pre-injected via `assemble_judge_context`)。**`confirm=true` 是显式不变量**: approve 不自动推进 —— `advance_lifecycle_to_target` 逐个状态推进,遇 `ui_button` 停住(`_story_state_gate.final_target`,确认后 `/lifecycle/advance` 续推)。Reject budget (`check_reject_budget`): ≤3 rejects per stage + each reject reason must differ from the prior, else force-escalate (防打回循环,§评审 A2)。`boundary_judge.py` 保留为兼容模块(planner 已不再调用);verify 的 `unified_gate` 仍走独立调用。
