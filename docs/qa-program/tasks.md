...
### T1.3 · no_progress 终止(防死循环) `[已完成]` · 走 A(更新 README)

**现状**(2026-07-08 已查证 + owner 裁定走 A 已执行):README 原「连续无改善自动终止」是 LangGraph
时代描述,`detect_no_progress` 已被 ISS-008 删,FC 靠 `max_retries` 计数兜底(T1.1 已验证)。
owner 走 A:已更新 `README.md` §收敛条件(删 no_progress/max_rounds,改为 max_retries + FC 说明)。
详见 [收尾报告](reports/T1.3-closed-by-doc-fix.md)。
