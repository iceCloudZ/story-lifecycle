...
- **★ judge 三元组原则（2026-08-06 迭代 1 收尾新增）**：一切评分/基线/验收标签只认 **opencode-go 端点 + deepseek-v4-flash + 当前 prompt** 这个三元组。历史基线（snapshot_20260805 的 conformance 分数）是 **DeepSeek 官方端点 + 旧 prompt** 评的，与 Go+新 prompt **不可直接比较**（实测同 diff+同参照物：旧三元组 align=2，新三元组 align=3-5）。规则：
