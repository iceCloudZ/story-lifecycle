...
随着系统从简单状态机走向 Orchestrator Agent，用户真正需要的不只是更多快捷键，而是一个能解释状态、汇总证据、提出下一步建议的控制台。这个控制台应该在用户最常停留的地方出现：Board 右侧。

因此新增一个 **Board Copilot Panel**：在 TUI 右侧固定区域中提供面向当前选中 Story 的 LLM 对话、运行态解释、受控动作建议和诊断打包入口。

它不是一个通用聊天窗口，也不是让 LLM 直接接管工作流。它是 Story Board 的“副驾驶”和“诊断台”。

核心原则：

```text
LLM explains / proposes
Policy and TUI confirm
Existing handlers execute
Event log records
```

也就是：LLM 可以解释、建议、生成待确认动作；真正修改配置、推进任务、恢复 Story、跳过阶段、失败 Story、打包日志，都必须经过确定性 handler 和可审计日志。

## 目标

P0 目标：

1. 在 `story board` 右侧新增 Copilot 面板。
2. 面板跟随当前选中 Story，展示当前阶段、状态、错误、最近事件和可用动作。
3. 提供受控对话入口：用户可以问“为什么卡住了”“下一步怎么办”“review 在对抗什么”。
4. 提供卡住时的一键诊断打包能力，生成可发给维护者排查的诊断包。
5. 所有建议和诊断动作写入 `event_log`，便于回放。

非目标：

1. P0 不做全局聊天助手。
2. P0 不让 LLM 直接调用 skip/fail/resume/setup 等 destructive 或 state-changing 操作。
3. P0 不做长期记忆、自动学习或 pattern 激活。
4. P0 不把完整业务源码、完整日志默认发给外部 LLM。
...
├──────────────────────────────────────────────┬───────────────────────────┤
│ Plan panel / notification                    │ Copilot panel             │
├──────────────────────────────────────────────┤                           │
...
左侧保留现有故事列表和操作习惯。右侧面板承担解释、建议和诊断。

建议快捷键：

| 快捷键 | 行为 |
|---|---|
| `tab` | 在 Story 列表和 Copilot 面板之间切换焦点 |
| `?` | 仍显示帮助 |
| `o` | 聚焦 Copilot 输入框 |
| `p` | 对当前 Story 打包诊断 |
| `shift+p` | 打包全局诊断 |
| `ctrl+enter` | 发送 Copilot 问题 |
| `y` | 对当前建议动作确认执行 |
| `esc` | 取消当前建议或回到列表焦点 |

P0 可以先不实现复杂焦点系统，只要右侧面板展示摘要，并提供 `p` 打包诊断和一个简单的 query dialog。后续再补真正的输入框。
...
右侧面板可以列出“可建议动作”，但不自动执行：

| 建议 | 执行方式 |
|---|---|
| 查看 detail | 调用现有 detail render |
| 进入终端 | 调用现有 `action_enter_terminal` |
| resume | 调用现有 `action_resume_story`，需确认 |
| skip stage | 调用现有 `action_skip_stage`，需确认 |
| fail story | 调用现有 `action_fail_story`，需确认 |
| run doctor | 调用现有 `action_run_doctor` |
| run setup | 调用现有 `action_run_setup`，需确认 |
| package diagnostics | 调用新增 diagnostics bundler |

P0 的 Copilot 只生成 `SuggestedAction`：
...
Copilot 每次回答只拿当前 Story 的 Debug Packet，不读取全仓库。
...
TUI 可以把 summary 和 evidence 渲染出来，把 suggestions 渲染成可确认动作。
...
| 类型 | 例子 | 是否可自动执行 |
|---|---|---|
| read_only | 状态解释、debug packet、诊断打包 | 可以 |
| local_config | run setup、修改 provider/model | 需要确认 |
| workflow_state | resume、skip、fail、abort | 需要确认 |
| destructive | delete story、清理目录 | P0 不允许由 Copilot 发起 |
| external_send | 上传日志、发给远程服务 | P0 不自动做，只生成本地包 |
...
src/story_lifecycle/
  cli/
    tui.py                    # 增加右侧 panel 和快捷键
    diagnostics.py            # CLI 命令入口
  orchestrator/
    copilot.py                # Debug Packet -> LLM response / rule response
...
TUI selected story
  ...
  -> optional ask_copilot(question, packet)
...
默认不调用 LLM。右侧面板根据 debug packet 做确定性解释。
...
Board Copilot Panel 是 Orchestrator Agent 的人机交互壳层，不是智能内核本身。

对应关系：

| Orchestrator Agent 概念 | Board Copilot 映射 |
|---|---|
| DecisionEnvelope | Copilot SuggestedAction |
...
这意味着右侧 Copilot 不需要等完整 Orchestrator Agent 落地。它可以先作为观测和诊断界面存在，后续逐步接入 Policy Engine 和 DecisionEnvelope。

## 版本拆分

### P0: 诊断优先

目标：先解决“用户卡住后维护者拿不到上下文”的问题。

内容：

1. 新增 `story diagnostics STORY_KEY`。
2. 新增 `story diagnostics --global`。
3. 新增 `orchestrator/diagnostics.py`，生成 zip。
4. TUI 右侧面板展示当前 Story 摘要和最近事件。
5. TUI `[p]` 对当前 Story 打包诊断。
6. 诊断包生成写入 `event_log`。
...
- 所有动作写 event_log。
- destructive 动作 P0 禁止由 Copilot 发起。
...
1. 先做 `story diagnostics STORY_KEY` 和 `--global`。
2. 再做右侧只读面板。
3. 再把 `[p]` 接到诊断打包。
4. 再做 Ask Copilot 的 LLM 问答。
5. 最后做 SuggestedAction 和 Policy Engine 集成。

原因：诊断打包的价值最高、风险最低，而且能直接解决真实用户卡住后排查成本高的问题。右侧面板先展示确定性事实，等事实管道稳定后再接 LLM，会更稳。
