...
`07-scenario-knowledge-workflow-design.md` refines this bootstrap design.

`story project init-knowledge` is now treated as the project overview bootstrap layer. It may generate coarse scenario candidates, service/API/table/MQ indexes, and an initial graph, but it is not required to generate complete business scenario documents.

Complete scenario documents such as registration, login, withdraw, credit, repay, overdue, advance repay, coolingoff, account recovery, and account logoff are generated by:

```bash
story project scenario scan <scenario-id>
```

This update resolves the earlier tension where `init-knowledge` appeared to own both overview bootstrap and deep scenario scanning. The scenario workflow, run draft protocol, graph patch schema, and merge gate are defined in `07-scenario-knowledge-workflow-design.md`.
...
External retrieval indexes such as CodeGraph, ast-grep indexes, Sourcegraph indexes, or local search caches are auxiliary providers only. They can help produce `extracted` facts, but they are not the knowledge system of record. The durable knowledge body remains `.story/knowledge/` text files with source-backed evidence.
...
.story/knowledge/scenarios/**                 # optional coarse candidates only
.story/knowledge/indexes/**                   # coarse overview indexes
.story/knowledge/graph/product-context-graph.json
.story/knowledge/reviews/pending-review-items.md
```

Business scenario deep-scan outputs are not part of the required bootstrap completion contract. They are produced later by `story project scenario scan <scenario-id>`.
